...
**Enhancements**: 100+ improvements across 17 phases

---

## What's New in the PGFX Edition

### Genre-Aware Song Architecture (90+ Structure Templates)
...
- **90+ templates**: Metal (8 variants), Reggae (3), K-Pop, Hip-Hop (7), Blues (6), Punk (6), Folk (5), DJ/Turntablism (4), Jazz (12 aliases), Traditional/World (4), plus genre-agnostic fallbacks.
- **DJ / Turntablism**, **Dual DJ**, and **Sample DJ / Crate Digger** as first-class genres with scratch effects, battle vocabulary, sample-flip structure, and turntablist/sampling BPM ranges.
- **Exact-match genre routing**: alias resolution uses exact matching — no more cross-genre vocabulary contamination ("dub" no longer pulls in dubstep, "r&b" no longer pulls in hip-hop).
- **4 Traditional/World Music Genres**: Klezmer, Mariachi, Bhangra, Andean — each with culturally-specific structure templates, vocabulary modules, BPM ranges, and sonic tag examples. (Genres that ACE-Step cannot authentically reproduce — such as Gagaku, Carnatic, Hindustani, Tuvan Throat Singing, and others — have been removed to ensure every listed genre produces what it promises.)
...
- **Per-Track Retry Buttons**: A failed track gets a ↻ audio retry button (regenerates just that track, then auto-attempts its video); a completed track missing its video gets a 🎬 button. No more re-running the whole album for one bad track.
- **90-Minute Client Poll**: The batch's job poll waits up to 90 minutes (matching the server's 45-min/phase cap) and clearly says the job continues server-side if it ever times out — no more false "Timed out after 15 minutes" failures.
...
- **Single-frame image prompts**: section images are explicitly prompted as ONE full-frame scene (never storyboards/multi-panel grids), with visual consistency across the series
- **Audio-visualizer fix**: `showfreqs` filter now uses the correct `colors=` option — eliminates random per-track "video failed to generate" errors
...
3. Replace/merge the following files from this PGFX Edition:
   ...
   - `server/services/` — **required** — merge the whole directory (`prompt-builder.mjs`, `comfyui-client.mjs`, `comfyui-bridge.mjs`, `comfyui-model-scanner.mjs`, `beat-detector.mjs`) — `server.mjs` imports these
...
| `server/services/prompt-builder.mjs` | Existing | ~535 lines | Cover art, singer image, video prompt builders — SINGLE IMAGE/SINGLE FRAME directives (no more storyboard/panel grids), Song moment + Visual consistency language for multi-section series |
| `ui/dist/album.html` | Modified | ~50 KB | Album Generator — 20-track workflow with multi-select genre picker (200+ genres, 17 categories), random genre-aware theme generator, gender/vocalist context fields, auto-fill, artist name, album title, persistent metadata, ZIP download with folder organization, 18-language selector, Album Library button in header, per-track Retry/Regenerate lyrics buttons |
...
See **[HOT-Step-Enhancements-Report.md](HOT-Step-Enhancements-Report.md)** for the complete technical documentation of all enhancement sections across 17 phases, reproduction guide, and file locations.
...
- **PyrateGFX Productions** — All the really cool enhancements.
...
This project builds upon **HOT-Step CPP by scragnog** (which uses the **ACE-Step engine under the MIT License**). The original HOT-Step CPP code and ACE-Step engine retain their original licenses.

The **PGFX Edition enhancements** in this repository are licensed under the **MIT License** — see the `LICENSE` file. See individual file headers for specific licensing of third-party components.

Third-party models, weights, and assets (including ACE-Step engine binaries, quantized GGUF checkpoints, and ComfyUI models) are **not** covered by this project's license and are subject to their own terms of use.
...
- **AI-generated content:** Tracks, album art, and videos produced by this tool are generated by AI models. You are solely responsible for how you use, publish, commercialize, or distribute AI-generated output, including clearing rights for any lyrics, names, or samples.
- **Not affiliated:** This project is an independent community edition. It is not affiliated with, endorsed by, or connected to scragnog, ComfyUI, or any model vendor unless explicitly stated.
- **Model licensing:** When downloading models through the Model Manager or running ComfyUI workflows, review each model's license before commercial use. Many open weights (e.g., Qwen, MiniMax, LTX, FLUX) have specific terms.
