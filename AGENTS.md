...
#### 16. Generation Timeout Overhaul — False "Timed out after 15 minutes" Failures ✅
- **User report:** Track 01 of an album run failed with `✗ 01. ... Failed: Timed out after 15 minutes` — BUT the job kept running and later tracks generated fine.
- **Root cause (traced by exact string):** "Timed out after 15 minutes" exists ONLY in `ui\dist\index.html` `pollJob()` — `var TIMEOUT = 15 * 60 * 1000` (was line 674) + `throw new Error('Timed out after 15 minutes')` (line 678). Client abandoned the `/api/generate` job poll at 15 min WITHOUT canceling; server job (`router6` jobs Map, no expiry) kept running → false failure. Album Creator (album.html) is a PROMPT MANAGER: Phase 1 lyrics via `/api/inspire/llm`, Phase 2 saves `localStorage['hs-album-pending']` + redirects to main page → main page generates audio via POST `/api/generate` + `pollJob`. So the main-page batch is the audio path; album.html has NO audio polling of its own.
- **Server /api/generate caps (fork @136141/136197):** `pollUntilDone` clamps `generationTimeoutMinutes` to [5,120], default 45 MIN PER PHASE (LM @136306, synth @136713). Client was 15 min total vs server 45+45 = 90 min envelope → mismatch.
- **Fix 1 (CLIENT, live immediately — no restart):** `ui\dist\index.html` `pollJob` TIMEOUT 15 → **90 min** (line ~678); error message now honest: "Timed out after 90 minutes (job continues on server — check the library before regenerating)". Served from disk on next page load.
- **Fix 2 (SERVER LLM, needs restart):** 5× `AbortSignal.timeout(9e5)` (15 min) in fork LLM providers (Ollama @154168, LMStudio, OpenAI, OpenAI-compat, llama.cpp) → new module-level `const LLM_TIMEOUT_MS = Number(process.env.LLM_TIMEOUT_MS) || 1800000` (30 min, env-overridable) inserted before ollama.ts module (~154103). `node --check` PASS.
- **Fix 3 (SERVER engine-LM inspire, needs restart):** `pollUntilDone2` MAX_POLLS 600 → **2400** (500 ms × 2400 = 20 min; was 5 min) — gates `/api/inspire` engine LM preview ("Inspire timed out" @297979). Fork @297961.
- **Mirrored to PORT TARGET (v1.2.2-PGFX):** upstream v1.2.2 used even TIGHTER 5-min caps — 8× `AbortSignal.timeout(3e5)` across all LLM providers (110961–111514) → same `LLM_TIMEOUT_MS` const (inserted before types.ts ~110772). Target `pollUntilDone2` MAX_POLLS 600 → 2400 (308971→~308976). Target already has the same 45-min/phase `/api/generate` clamp (@167706, @167760) — no change needed. `node --check` PASS on both files.
- **Deliberately NOT changed:** undici bodyTimeout (moot — streaming readers keep chunks flowing; real cap was the AbortSignal; custom dispatcher needs `undici` import unavailable in bundle context). Anthropic 5-min cap (remote API, different class). Server 45-min/phase guard stays (legitimate server-side abort).
- **User guidance given:** current run's "failed" track audio is in the library server-side; refresh page for client fix; restart server (when nothing generating) for server fixes; optional `LLM_TIMEOUT_MS` in .env (default 30 min fine).

### Session Summary — 2026-07-31 (continued)

#### 17. Image/Video Fixes + Album Batch Per-Track Retry (this session) ✅
- **Bug: multi-panel grid images (HIGH IMPACT).** FLUX.2 Klein renders "chapter/story/narrative" language as 4–6 panel storyboards — wrong for album art AND music-video section frames. Root cause: BOTH prompt builders in `server/services/prompt-builder.mjs` (fork + target v1.2.2) carried narrative triggers.
  - `buildCoverArtPrompt` (sectionIndex branch ~435): old "early chapter — introducing the world" arcDesc + "one continuous visual story" + "Narrative progression:" → new "Song moment: an opening/building/peak/intensifying/closing moment" + "Visual consistency: same characters, same location, same lighting palette as the other section images in this series (each section image is generated separately as its own single image)" + **"SINGLE IMAGE, SINGLE FRAME: one full-frame composition showing one scene only. Not a storyboard, not a comic strip, not multiple panels, not a grid of images, not a sequence, not a collage. This image stands alone."**
  - `buildSingerImagePrompt` (~479): old "Story arc:"/"Narrative progression:"/"cohesive visual narrative" + chapter descs → same "Song moment: {position}, {energy} — {arc}" + Visual consistency + SINGLE IMAGE lines; trailer dropped "cohesive visual narrative".
- **Bug: random "video failed to generate" per-track (HIGH IMPACT).** Evidence in `logs/2026-07-31_14-05-09/node_console.log`: `[VideoCreate] ffmpeg error: Error applying option 'color' to filter 'showfreqs': Option not found`. Fork `vizModes` used invalid `color=` — correct option is **`colors=`** (verified against bundled ffmpeg: `server/ffmpeg.exe -h filter=showfreqs` shows `colors`, default "red|green|blue|yellow|orange|lime|pink|magenta|brown"). `showfreqs` was the primary viz mode for any track whose FIRST section mapped to it (build/`primaryVizIdx`), so failure looked random (showwaves/showspectrum tracks succeeded). Fixed `color=green`→`colors=green` (verse) and `color=yellow`→`colors=yellow` (build) in fork (~299048) AND target (~310103/310107).
- **Negative prompt hardening (both servers):** cover-art `-n` now includes "storyboard, comic strip, multiple panels, split panel layout, grid layout, sequence of images, collage" (~42535 fork / ~47820 target). NOTE: ComfyUI mode ignores negatives (ConditioningZeroOut) — the single-frame fix carries the weight there; local sd-cli mode gets both.
- **FEATURE: Album Batch per-track Retry buttons (fork `ui/dist/index.html`):** user could only retry a failed album track by re-running the whole album. Now:
  - **↻ audio retry** (`.ab-r-audio`) on error cards → re-runs `POST /api/generate` for THAT track via extracted `generateTrackAudio(t,i)` (was the inline batch-loop body), then auto-generates video if audio succeeds.
  - **🎬 video button** (`.ab-r-video`) on done-tracks without a video → re-runs `/api/inspire/video/create` for that track via extracted `generateTrackVideo(t,i)`.
  - Single-flight `retryingTrack` flag (same pattern as `generating`); buttons hidden while batch/retry is running; stale `_videoUrl` cleared on audio retry so old video can't pair with new audio; `_error`/`_videoError` recorded + cleared.
  - Video button gated on `t._audioUrl` (nothing to attach otherwise — caught by functional test).
  - **Verified:** 26/26 assertions in a DOM/fetch-stubbed harness running the REAL inline script: error→retry→done(+video), video-only retry, button visibility matrix (error-only audio btn, no-audio hides video btn, hidden during batch), wired onclick click test, full 3-track batch, mid-batch failure doesn't abort the rest. All 12 inline scripts `new Function`-clean; `node --check` PASS on both server.mjs + both prompt-builder.mjs; `rg "color=(green|yellow)"` = 0 in both servers.
- **Deploy note:** all fixes are client/static or server-file — restart the server (when idle) to load new `server.mjs`/`prompt-builder.mjs`; refresh page for the new `index.html`.

### Relevant Files
- `server/server.mjs` — All edits: GENRE_STRUCTURE_TEMPLATES, GENRE_HOOK_TIMING, GENRE_VOCABULARY_MODULES, resolveGenreFromStyles, INSTAGEN_FULL_SYSTEM_PROMPT, OutroFix (Step 2b ~45718), CHORUS VARIATION (~47901), sampledj module (~44672) + routing (~46045) + BPM (~46186) + template (~46616), mergeGenreModules allowSampleEffects (~46363), FLEXIBLE_VERSE_GENRES (~47398/~154798), allowSampleEffects hints (~298389), creativity handler (~298071), LLM_TIMEOUT_MS const (~154103) + 5 provider timeout swaps, pollUntilDone2 MAX_POLLS 2400 (~297961)
- `ui/dist/index.html` — Floating buttons/pill/creativity slider (~28–1600), pollJob TIMEOUT 15→90 min (~674/678), album batch per-track retry: render buttons (~604–610), wiring (~641–650), extracted generateTrackAudio/generateTrackVideo/retryTrackAudio/retryTrackVideo (~1052–1193)
...
9. **Timeout layering:** client wait caps must be ≥ server per-phase caps (server /api/generate = 45 min/phase, 2 phases). Server jobs are NOT cancelled when a client abandons its poll — never treat a client-side timeout as a failed job, and always message accordingly.
10. **Per-track retry over whole-album rerun:** never force a user to re-run a whole batch for one bad track. Extract the per-track body into a reusable function + single-flight guard, gate video affordances on the presence of audio, and always clear stale artifacts (`_videoUrl`) before regenerating so old output can't pair with new audio.
11. **Test the real script, not a copy:** a DOM/fetch stub harness that executes the actual inline script (with the final `tryLoadTemplate();})();` replaced by an in-scope test block) catches scope/hoisting/wiring bugs that syntax checks miss — e.g. it caught the video-button-without-audio guard gap.
